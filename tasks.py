...
NAME = os.getenv("IMAGE_NAME", "axians-netbox-plugin-pdu")
PWD = os.getcwd()

COMPOSE_FILE = "development/docker-compose.yml"
BUILD_NAME = "axians_netbox_pdu"
...
def nbshell(context, netbox_ver=NETBOX_VER, python_ver=PYTHON_VER):
    ...
    """
    context.run(
        f"docker-compose -f {COMPOSE_FILE} -p {BUILD_NAME} run --rm netbox python manage.py nbshell",
        env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver},
        pty=True,
    )
...
def cli(context, netbox_ver=NETBOX_VER, python_ver=PYTHON_VER):
    ...
    """
    context.run(
        f"docker-compose -f {COMPOSE_FILE} -p {BUILD_NAME} run --rm netbox bash",
        env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver},
        pty=True,
    )
...
def create_user(context, user="admin", netbox_ver=NETBOX_VER, python_ver=PYTHON_VER):
    ...
    """
    context.run(
        f"docker-compose -f {COMPOSE_FILE} -p {BUILD_NAME} run --rm netbox python manage.py createsuperuser --username {user}",
        env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver},
        pty=True,
    )
...
def makemigrations(context, name="", netbox_ver=NETBOX_VER, python_ver=PYTHON_VER):
    ...
    if name:
        context.run(
            f"docker-compose -f {COMPOSE_FILE} -p {BUILD_NAME} run --rm netbox python manage.py makemigrations --name {name}",
            env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver},
        )
    else:
        context.run(
            f"docker-compose -f {COMPOSE_FILE} -p {BUILD_NAME} run --rm netbox python manage.py makemigrations",
            env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver},
        )
